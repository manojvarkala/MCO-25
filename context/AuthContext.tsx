        ...
        if (payload.user) {
            ...
            if (!isSyncOnly) {
                try {
                    ...
                    // 1. Session Expired (Normal, just needs refresh)
                    if (serverError.code === 'jwt_auth_expired_token' || serverError.message?.includes('expired')) {
                        console.warn("Login Aborted: Token expired.");
                        throw new Error("Session Expired"); 
                    }

                    // 2. Authorization Header Missing (Server Config Issue)
                    if (serverError.code === 'jwt_auth_missing_token' || serverError.message?.toLowerCase().includes('authorization header')) {
                    ...
                    // 3. Invalid Token (Secret Key Mismatch)
                    if (serverError.code === 'jwt_auth_invalid_token' || serverError.message?.includes('Invalid token')) {
                        console.error("Login Aborted: Token rejected by server.");
                    ...
                    // 4. Other non-critical errors (Network, etc) - Allow login to proceed in offline mode if needed
