...
const ProductEditorModal: FC<ProductEditorModalProps> = ({ product, allProducts, onSave, onCancel, isSaving }) => {
    const [formData, setFormData] = useState<EditorState>(product);
    ...
    const simpleAndSubProducts = allProducts.filter(p => p.type === 'simple' || p.type === 'subscription');
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-[rgb(var(--color-card-rgb))] rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 overflow-y-auto space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="salePrice" value={formData.salePrice || ''} onChange={handleChange} required className="w-full p-2 border rounded-md" step="0.01" />
                        </div>
                        <div>
                            <label className="text-sm font-bold block mb-1">Regular Price ($)</label>
                            <input type="number" name="regularPrice" value={formData.regularPrice || ''} onChange={handleChange} placeholder="Optional" className="w-full p-2 border rounded-md" step="0.01" />
                        </div>
                    </div>
                            ...
                            <div className="max-h-48 overflow-y-auto space-y-2 p-2 bg-slate-100 rounded-md">
                                {simpleAndSubProducts.map(p => (
                                    <label key={p.sku} className="flex items-center gap-2 text-sm p-1">
                                        ...
                                        />
                                        <span>{stripHtml(p.name)} ({p.sku})</span>
                                    </label>
                                ))}
                            </div>
                        </div>
                ...
                <div className="flex justify-end gap-3 p-4 border-t border-[rgb(var(--color-border-rgb))]">
                    ...
                    <button onClick={() => onSave(formData)} disabled={isSaving} className="flex items-center gap-2 px-4 py-2 bg-green-500 rounded-lg font-semibold text-white hover:bg-green-600 disabled:bg-slate-400">
...
const ProductCustomizer: FC = () => {
    ...
    const [editingProduct, setEditingProduct] = useState<EditorState>(null);
    ...
    const handleSave = async (productData: EditorState) => {
        ...
        setIsSaving(true);
        try {
            // Ensure bundledSkus is an array if type is bundle, otherwise undefined
            const finalData = {
                ...productData,
                        ...
                        {productsToDisplay.map(product => (
                            <div key={product.sku} className="editable-card">
                                ...
                                <div className="editable-card__content text-sm">
                                    ...
                                    {product.regularPrice && parseFloat(product.regularPrice) > 0 && <div className="flex justify-between"><span>Regular Price:</span> <span className="font-semibold">${product.regularPrice}</span></div>}
