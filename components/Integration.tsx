...
function mco_get_certificate_data_callback($request) {
    ...
    $result = $all_results[$test_id];
    $candidate_name = trim($user->first_name . ' ' . $user->last_name) ?: $user->display_name;
    
    // The app has the full template/exam data and will assemble the final certificate data object.
    ...
    <div class="mco-user-details" style="font-family: sans-serif; border: 1px solid #ddd; padding: 20px; margin-bottom: 20px; background: #f9f9f9;">
        ...
        <?php if (class_exists('WooCommerce')) {
            ...
            if ($customer_orders) {
                foreach ($customer_orders as $order) {
                    foreach ($order->get_items() as $item) {
                        if ($product = $item->get_product()) $purchased_skus[] = $product->get_sku();
                    }
                }
            }
            $paid_exam_ids = array_values(array_intersect($all_exam_skus, array_unique($purchased_skus)));
        ...
        if (!empty($results) && is_array($results)) {
            echo '<table style="width: 100%; border-collapse: collapse;"><tr><th style="border: 1px solid #ddd; padding: 8px; text-align: left;">Exam ID</th><th style="border: 1px solid #ddd; padding: 8px; text-align: left;">Score</th><th style="border: 1px solid #ddd; padding: 8px; text-align: left;">Date</th></tr>';
            foreach ($results as $result) {
                $date = date('Y-m-d H:i', $result['timestamp'] / 1000);
                echo '<tr><td style="border: 1px solid #ddd; padding: 8px;">' . esc_html($result['examId']) . '</td><td style="border: 1px solid #ddd; padding: 8px;">' . esc_html($result['score']) . '%</td><td style="border: 1px solid #ddd; padding: 8px;">' . esc_html($date) . '</td></tr>';
            }
            echo '</table>';
